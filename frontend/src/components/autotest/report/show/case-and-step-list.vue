...
const reRun = (row: { id: undefined, case_id: undefined, case_data?: {id?: undefined}, suite_id: undefined, children:[{id?: undefined, case_id?: undefined, case_data?: {id?: undefined}, result: string}]}, option: string = 'all') => {
  const reRunIdList: number[] = []
  if (row.suite_id) {
    const currentCaseId = row.case_id ?? row.case_data?.id ?? row.id
    if (currentCaseId !== null && currentCaseId !== undefined) {
      reRunIdList.push(currentCaseId as number)
    }
  } else {
    row.children.forEach(child => {
      if (option === 'all' || option === 'failed' && ['fail', 'error'].includes(child.result) || option === 'success' && child.result === 'success') {
        const childCaseId = child.case_id ?? child.case_data?.id ?? child.id
        if (childCaseId !== null && childCaseId !== undefined) {
          reRunIdList.push(childCaseId as number)
        }
      }
    })
  }
