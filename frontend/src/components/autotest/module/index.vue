<template>
  <div class="layout-container autotest-module-shell module-page">

    <div class="layout-container-form flex space-between module-page-toolbar">
      <div class="layout-container-form-handle">
        <el-form label-width="60px" inline class="module-filter-form">
    ...
    <div class="module-page-body autotest-module-body">
      <el-row>

        <!-- 第一列模块树 -->
        <el-col style="width: 20%; margin-left: 10px; border:1px solid;border-color: #ffffff rgb(234, 234, 234) #ffffff #ffffff;">
          <el-tabs v-model="tabActiveName" class="table_padding table_project autotest-tabs-header-compact">
            <el-tab-pane name="moduleTree">
              <template #label>
                <span class="autotest-tab-label">
                  <span class="autotest-tab-title">模块树</span>
                  <el-tooltip content="在当前服务下添加一级模块" placement="top" :show-after="400">
                    <el-button
                        v-show="queryItems.project_id"
                        type="primary"
                        size="small"
                        class="autotest-tab-primary-btn"
                        @click.stop="addParentModule"
                    >
...
// 顶部筛选条：表单项/按钮留白，与下方模块树+接口列表拉开间距
.module-page {
  > .layout-container-form.module-page-toolbar {
    padding-bottom: 12px;
    margin-bottom: 4px;
  }
...
.module-filter-form.el-form--inline {
  display: flex;
  flex-wrap: wrap;
  align-items: center;
  gap: 4px 0;
...
.module-filter-form :deep(.el-form-item) {
  margin-right: 20px;
  margin-bottom: 0;
}

.module-filter-form :deep(.el-form-item:last-child) {
  margin-right: 0;
}

.module-toolbar-btn + .module-toolbar-btn {
  margin-left: 10px;
}

.module-toolbar-btn:first-of-type {
  margin-left: 12px;
}
