...
watch(() => props.caseSuiteTree, (newValue) => {
  if (newValue && newValue.length > 0) {
    syncCheckedSuites()
    const preferredSuite = newValue.find((item: any) => item.suite_type === 'process') || newValue[0]
    if (preferredSuite && preferredSuite.id && currentTreeDataId.value !== preferredSuite.id) {
      currentTreeDataId.value = preferredSuite.id
      getCaseList(preferredSuite.id)
    }
  } else {
    tableDataList.value = []
  }
...
watch(() => props.suiteIdList, (newValue, oldValue) => {
  tempSuiteIdList.value = Array.isArray(newValue) ? newValue.slice() : []
  syncCheckedSuites()
})
...
const syncCheckedSuites = () => {
  nextTick(() => {
    const treeRef: any = suiteTreeRef.value
    if (treeRef && typeof treeRef.setCheckedKeys === 'function') {
      treeRef.setCheckedKeys(tempSuiteIdList.value || [])
    }
  })
}
...
const onCheckChange = (data: { id: any }, checked: boolean) => {
  if (checked && currentTreeDataId.value !== data.id) {
    currentTreeDataId.value = data.id
    getCaseList(data.id)
  }
}
