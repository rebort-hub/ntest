<template>
    <div class="case-list-wrap">
      <el-tabs v-model="activeTab" class="case-list-tabs autotest-tabs-header-compact">
        <div
        ...
        <el-tab-pane name="case">
          <template #label>
            <span class="autotest-tab-label">
              <span class="autotest-tab-title">用例列表</span>
              <el-tooltip content="在当前用例集下新增用例" placement="top" :show-after="400">
                <el-button
                    v-show="queryItems.suite_id"
                    type="primary"
                    size="small"
                    class="autotest-tab-primary-btn"
                    @click.stop="showEditDrawer('add', undefined)"
                >
...
.case-batch-toolbar :deep(.el-button) {
  margin-left: 0;
}

// 勾选列与复选框对齐（全局 autotest-data-table 已统一斑马线与边框）
:deep(.autotest-data-table) {
