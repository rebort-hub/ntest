<template>
  <div class="layout-container autotest-module-shell case-suite-page">

    <div class="layout-container-form flex space-between case-suite-toolbar">
      <div class="layout-container-form-handle">
        <el-form label-width="60px" inline class="case-suite-filter-form">
    ...
    <div class="autotest-module-body">
      <el-row class="autotest-module-split-row autotest-suite-split">

        <!-- 第一列用例集树 -->
        <el-col class="autotest-split-sidebar">
          <el-tabs v-model="tabActiveName" class="table_padding table_project suite-tree-tabs autotest-tabs-header-compact">
            <el-tab-pane name="moduleTree">

              <template #label>
                <span class="autotest-tab-label">
                  <span class="autotest-tab-title">用例集树</span>
                  <el-tooltip content="在当前项目下添加一级用例集" placement="top" :show-after="400">
                    <el-button
                        v-show="queryItems.project_id"
                        type="primary"
                        size="small"
                        class="autotest-tab-primary-btn"
                        @click.stop="addParentCaseSuite"
                    >
                      <span class="autotest-tab-btn__inner">
                        <Plus theme="outline" :size="15" class="autotest-tab-btn__icon" />
                        <span>新增用例集</span>
                      </span>
                    </el-button>
                  </el-tooltip>
                  <el-tooltip content="从文件导入用例集" placement="top" :show-after="400">
                    <el-button
                        v-show="queryItems.project_id"
                        type="primary"
                        plain
                        size="small"
                        class="autotest-tab-secondary-btn"
                        @click.stop="uploadCaseSuite"
                    >
                      <span class="autotest-tab-btn__inner">
                        <UploadOne theme="outline" :size="15" class="autotest-tab-btn__icon" />
                        <span>导入用例集</span>
                      </span>
                    </el-button>
                  </el-tooltip>
              ...
              <div class="suite-tree-pane">
                <el-input v-model="filterText" class="suite-tree-filter-input" placeholder="输入关键字进行过滤" />
        ...
        <!-- 第二列，用例列表 -->
        <el-col class="autotest-split-main">
          <!-- 用例管理 -->
...
// 顶部「服务」选择与下方分栏留白
.case-suite-page {
  > .layout-container-form.case-suite-toolbar {
    padding-bottom: 12px;
    margin-bottom: 4px;
  }

  .autotest-module-body {
    margin-top: 8px;
  }
}

.case-suite-filter-form.el-form--inline :deep(.el-form-item) {
  margin-right: 0;
  margin-bottom: 0;
}

// 左侧用例集树：标题行与过滤输入、树之间留白
.suite-tree-tabs {
