...
const onColorChange = () => {
  ...
  // 更新 Element Plus 与全局主题色变量（与 theme/index.scss、Header 主题设置一致）
  document.documentElement.style.setProperty('--el-color-primary', primaryColor.value);
  document.documentElement.style.setProperty('--system-primary-color', primaryColor.value);
...
const loadConfig = () => {
  ...
  if (saved) {
    try {
      ...
      primaryColor.value = config.primaryColor || '#20a162';
