...
from .webdriver_action import get_web_driver
...
class Runner:
    ...
    async def init_client_session(self):
        ...
        if self.client_session is None:
            ...
            elif self.run_type in ("ui", "web", "web_ui"):
                self.client_session = UIClientSession()
                try:
                    self.client = await get_ui_client(browser_name=self.browser_name or "chrome")
                except Exception as error:
                    msg = str(error)
                    if "Executable doesn't exist" in msg or "browserType.launch" in msg:
                        raise RuntimeError("Playwright 浏览器可执行文件不存在，请先安装浏览器运行时（`playwright install`）")
                    raise RuntimeError(f"Playwright 初始化失败：{msg}")
            else:
    ...
    def try_close_browser(self):
        ...
        # 可能出现没有获取到driver的情况
        try:
            target = self.client if self.run_type == "ui" else self.driver
            if target:
                close_all = getattr(target, "close_all", None)
                if close_all:
                    if self.run_type == "ui":
                        return asyncio.create_task(close_all())
                    close_all()
    ...
    async def _run_test(self, step_dict):
        ...
        if self.run_type == "api":
            ...
            active_client = self.client if self.run_type == "ui" else self.driver
            ...
            # 数据提取
            await self.report_step.test_is_start_extract()
            extracted_variables_mapping = await extract.extract_data(
        ...
        try:
            await self.session_context.validate(
                ...
                client=self.client if self.run_type == "ui" else self.driver
            )
    ...
    async def run_step(self, step_dict, report_step_model):
        ...
        try:
            ...
            if "Could not find a connected" in str(error):  # app
