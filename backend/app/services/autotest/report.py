...
async def get_report_rerun_case_list(request: Request, form: schema.GetReportRerunCaseForm = Depends()):
    ...
    report_model = ApiReport if request.app.test_type == "api" else AppReport if request.app.test_type == "app" else UiReport
    report_case_model = ApiReportCase if request.app.test_type == "api" else AppReportCase if request.app.test_type == "app" else UiReportCase
    report_step_model = ApiReportStep if request.app.test_type == "api" else AppReportStep if request.app.test_type == "app" else UiReportStep

    report = await report_model.filter(id=form.id).first().values("run_type")
    if report["run_type"] == "api":
        # 跑接口产生的报告，重跑对象来自 report_step 的 element_id
    ...
    case_list = await report_case_model.filter(report_id=form.id, **filters).all().values("case_id")
