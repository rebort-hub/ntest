...
def get_swagger_data(swagger_addr):
    ...
    try:
        # 添加超时设置和更好的错误处理
        response = requests.get(swagger_addr, verify=False, timeout=30)
        response.raise_for_status()  # 如果状态码不是200会抛出HTTPError
        return response.json()
    except requests.exceptions.ConnectionError:
        raise requests.exceptions.ConnectionError(f"无法连接到Swagger服务：{swagger_addr}")
    except requests.exceptions.Timeout:
        raise requests.exceptions.Timeout(f"请求Swagger服务超时：{swagger_addr}")
    except requests.exceptions.HTTPError as e:
        raise requests.exceptions.HTTPError(f"HTTP请求错误：{e.response.status_code} - {e.response.reason}")
    except json.JSONDecodeError:
        raise json.JSONDecodeError("返回的数据不是有效的JSON格式", "", 0)
...
class PullApiFox:
    ...
    def parse_json_schema(self):
        ...
        def parse_schema(schema):
            data, required_list, schema_data_type = {}, schema.get("required", []), schema["type"]
            ...
            for field, field_object in schema.items():
                ...
                elif field_object["type"] not in ["object", "array"]:
    ...
    async def pull_api_detail(self):
        api_detail_response = await self.pull_data("https://api.apifox.com/api/v1/api-details?locale=zh-CN")
        api_dict = {}
        for api_item in api_detail_response:
            params, data_json, data_form = [], {}, [{"key": None, "value": None, "remark": None, "data_type": None}]
            body_type = api_item.get("requestBody", {}).get("type")
            for item in ["path", "query"]:
                for data in api_item.get("parameters", {}).get(item, []):
                    params.append({
                        "key": data["name"],
                        "value": "",
            ...
            if body_type == "application/json":
                req_schema = api_item.get("requestBody", {}).get("jsonSchema", {})
                if req_schema.get("type") == "array":
                    ref = req_schema.get("items", {}).get("$ref")
                    data_json = [self.parsed_json_schema.get(int(ref.split("/")[-1]), {})] if ref else []
                else:
                ...
                data_form = [{
                    ...
                    "value": "",
                    "data_type": item["type"],
                    "remark": f'{item["type"]}; {"必填" if item["required"] else "非必填"}; {item.get("description")}'
                } for item in api_item.get("requestBody", {}).get("parameters", [])]
                if not data_form:
                    data_form = [{"key": None, "value": None, "remark": None, "data_type": None}]

            response_schema = (api_item.get("responses") or [{}])[0].get("jsonSchema", {})
            if response_schema.get("type") in ["integer", "string"]:
                responses = {}
            ...
            api_dict[api_item["id"]] = {
                ...
                "params": params,
                "data_json": data_json if isinstance(data_json, (dict, list)) else {},
                "data_form": data_form,
                "response": responses
            }
...
async def pull_swagger(request: Request, form: schema.SwaggerPullForm):
    ...
    pull_log = await SwaggerPullLog.model_create(
        {"project_id": project.id, "pull_args": options, "desc": form.cookies},
        request.state.user
    )
    swagger_data = {}
    if form.cookies:
        try:
            ...
            tree = await pull_api_fox.pull_api_tree()
            await pull_api_fox.pull_api_detail()

            # 用当前项目更新模块和接口
            module_nodes = tree.get("children", []) if isinstance(tree, dict) else tree
            if isinstance(module_nodes, list) and module_nodes and isinstance(module_nodes[0], dict) and "children" in module_nodes[0] and "api" not in module_nodes[0]:
                # 兼容返回最外层为项目节点列表的结构
                module_nodes = module_nodes[0].get("children", [])
            module_index = 0
            for module_data in module_nodes:
                db_module = await ApiModule.filter(name=module_data["name"], project_id=project.id).first()
                ...
                for api_index, api_data in enumerate(module_data.get("children", [])):
                    ...
                    payload = {
                        ...
                        "project_id": project.id,
                        "module_id": db_module.id,
                        "mock_response": pull_api_fox.api_args_dict.get(json_api_id, {}).get("response", {}),
                        **pull_api_fox.api_args_dict.get(json_api_id, {})
                    }
                    if api is None:
                        await ApiMsg.create(**payload)
                    else:
                        await ApiMsg.filter(id=api.id).update(**payload)

            await pull_log.pull_success(project)
            return request.app.success("Apifox 数据拉取并更新完成")
        except Exception as error:
            await pull_log.pull_fail(project, str(error))
            return request.app.fail(f"apifox数据拉取报错，结果为: \n{error}")

    try:
        swagger_data = get_swagger_data(project.source_addr)  # swagger数据
        status = swagger_data.get("status")
        if status and status >= 400:
            await pull_log.pull_fail(project, swagger_data)
            error_msg = swagger_data.get("message", swagger_data.get("error", "未知错误"))
            return request.app.fail(f"Swagger数据拉取失败！\n错误原因：{error_msg}\n状态码：{status}\n\n请检查：\n1. Swagger地址是否正确\n2. 服务是否正常运行\n3. 网络连接是否正常")
        ...
        await pull_log.pull_fail(project, swagger_data)
        return request.app.fail(f"Swagger数据拉取失败！\n错误原因：HTTP请求错误\n\n请检查：\n1. Swagger地址是否正确\n2. 服务是否返回有效的Swagger文档\n\n技术详情：{str(error)}")
    except json.JSONDecodeError as error:
        await pull_log.pull_fail(project, swagger_data)
        return request.app.fail(f"Swagger数据拉取失败！\n错误原因：返回的数据不是有效的JSON格式\n\n请检查：\n1. Swagger地址返回的是否为JSON格式的数据\n2. 服务是否正确配置了Swagger文档\n\n技术详情：{str(error)}")
    except Exception as error:
        await pull_log.pull_fail(project, swagger_data)
        return request.app.fail(f"Swagger数据拉取失败！\n错误原因：{str(error)}\n\n请检查：\n1. Swagger地址格式是否正确（如：http://localhost:8080/v2/api-docs）\n2. 目标服务是否正常运行\n3. 网络连接是否正常\n4. Swagger文档格式是否符合规范")
    await pull_log.pull_success(project)
