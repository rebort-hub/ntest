...
class BaseTask(BaseModel):
    ...
    is_send = fields.CharEnumField(
        SendReportTypeEnum, default=SendReportTypeEnum.NOT_SEND.value, description="是否发送报告，not_send/always/on_fail")
    ...
    receive_type = fields.CharEnumField(
        ReceiveTypeEnum, default=ReceiveTypeEnum.DING_DING.value, description="接收测试报告类型: ding_ding、we_chat、email")
    ...
    status = fields.CharEnumField(
        DataStatusEnum, default=DataStatusEnum.DISABLE.value, description="任务的启用状态, enable/disable，默认disable")
