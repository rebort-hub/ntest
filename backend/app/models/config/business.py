...
class BusinessLine(NumFiled):
    ...
    receive_type = fields.CharEnumField(
        ReceiveTypeEnum, default=ReceiveTypeEnum.NOT_RECEIVE.value,
    ...
    bind_env = fields.CharEnumField(
        BusinessLineBindEnvTypeEnum, default=BusinessLineBindEnvTypeEnum.HUMAN.value, description="绑定环境机制，auto：新增环境时自动绑定，human：新增环境后手动绑定")
